    ...
    <div style="background-color:#f5f5f5;height:100%;">
            ...
            <div class="help-middle">
                <div class="middle-block" v-for="(item,index) in listQuestion" :key="index" >
                    ...
                    <div class="middle-left" @click="skip(index)" >
                        ...
                        <span class="middle-text"> {{name}}{{item.value}}</span>
                    </div>
        ...
        <div v-if="!show">
            <div class="helpt-top" @click="back()">
                <span class="topt-text">{{name}}{{listQuestion[questionNum].value}}</span>
            </div>
    ...
    export default {
		data() {
			return {
                ...
                listAnswer: [
                    {value:"是专注为中国大陆地区互联网用户提供的智能终端的回收服务平台。"},
                    {value:"回收市场上多种品牌的智能手机及平板电脑。"},
                    {value:"用户可以在首页点击‘马上评估’，根据产品流程完善设备信息，提交订单后系统会在30分钟内给出报价。"},
                    {value:"给出的价格为综合机器品牌、使用时间等信息后，给出的综合评估价格，不接受议价。"},
                    {value:"已接入数帮保险，机器确为运输途中损坏的，数帮保险为您承保订单金额50%的货款。"},
                    {value:"为线上服务平台，不接受用户自行上门送货。"},
                    {value:"脏机（偷捡抢）、黑机（苹果套保机），不回收；不在回收类别中的设备，不回收。"},
                    {value:"仅限回收当前登录设备，如您有多部设备，需在待回收的设备上安装app进行操作。"},
                    {value:"快递费用由用户自行承担，不接收到付的快递订单。"},
                    {value:"会在用户确认接受评估价格后24小时内，将订单金额的30%作为首付款，汇至用户已绑定的银行卡上; 在收到用户设备并检测后，将尾款汇至用户的银行卡上。"},
                    {value:"收到用户的机器后会进行检测, 机器需要与提交订单时硬件检测的信息一致，对于不一致的机器，平台拒绝回收。"},
                    {value:"对于未能通过检测的机器，会将机器快递给用户，运费需要用户自行承担。"},
                    {value:"需要退回的机器，平台使用顺丰快递快递给您，通常在3-5个工作日可以到达。"},
                ],
                questionNum: '',
                show: true
            }
        ...
        methods: {
            getPackagename(){ //获取参数
                ...
                this.$ajax.post(url,qs.stringify(params)).then( res => {
                    ...
                    console.log(this.$refs.we.length)
                    this.$refs.toptext.style.color = '#'+jsonParameter.color
                    for(var i = 0; i<this.$refs.we.length;i++){
                         this.$refs.we[i].style.color = '#'+jsonParameter.color
                    }
